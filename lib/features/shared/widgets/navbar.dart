...
class _NavbarState extends State<Navbar> {
  ...
  List<Map<String, dynamic>> get _guideData {
  ...
  void _startAutoVanishTimer() {
    ...
    _autoVanishTimer = Timer(const Duration(seconds: 5), () {
      if (mounted && _showGuide) {
        if (_guideIndex < _guideData.length - 1) {
  ...
  Widget build(BuildContext context) {
    ...
    final isLastStep =
        _guideData.isNotEmpty && _guideIndex == _guideData.length - 1;
    ...
    return Stack(
      children: [
        ...
        // ── Guide Overlay ───────────────────────────────────────────────────
        if (_showGuide && _guideData.isNotEmpty)
          AnimatedOpacity(
  ...
  Widget _navItem(int index, String label, String icon, {String? badge}) {
    ...
    return GestureDetector(
      onTap: () {
        ...
        final targetBranch = _tabToBranch(index, isCoach);
